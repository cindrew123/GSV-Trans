...
from .adapt_multi_trans_model import Multi_Transformer as Multi_Transformer
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        multi_transformer = Multi_Transformer(self.config_multi_trans, self.depth, img_size=512, vis=False).cuda()
        x_muti_trans, camone_multi, camsix_multi = multi_transformer(x0, complex_x) # x_muti_trans[2, 24, 32, 32]
        #time_end = time.time()
        #time_c= time_end - time_start
        #print('time cost', time_c, 's')
        x, attn_weights, features, camone_vb, camsix_vb, attn = self.transformer(x0,complex_x)  # x[2, 1024, 768]
        x_all = self.fuse(x,x_muti_trans) # [2, 1024, 768] & [2, 24, 32, 32] --> [2, 1024, 768]
        #print (x.shape)
        x_all = self.decoder(x_all, features) #[2, 1024, 768]-->[2, 16, 512, 512]
        
        logits = self.segmentation_head(x_all)#[2, 16, 512, 512] -- >[2, 6, 512, 512]
        #print (logits.shape)
        
        pseudo_cam = self.Pseudo_Cam(camone_vb, camsix_vb, camone_multi, camsix_multi)
        # aff

        return logits, pseudo_cam, camsix_vb, camsix_multi
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states,complex_x):
        ...
        return encoded, attn_weights, camsone,camssix, attns
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids,complex_x):
        ...
        encoded, attn_weights, camone, camsix, attns = self.encoder(embedding_output,complex_x)  # (B, n_patch, hidden)
        # aff
        return encoded, attn_weights, features, camone, camsix, attns
